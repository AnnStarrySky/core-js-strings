...
function sumOfCodes(str) {
  ...
  return result.reduce((acc, el) => acc + el.charCodeAt(), 0);
}
...
function findLongestWord(sentence) {
  const arrSort = sentence.split(' ').sort((a, b) => b.length - a.length);
  return arrSort[0];
}
...
function encodeToRot13(str) {
  ...
  const nop = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm'.split('');
  return str
    .split('')
    .map((el) => {
      const index = abc.indexOf(el);
      return index !== -1 ? nop[index] : el;
    })
    .join('');
}
